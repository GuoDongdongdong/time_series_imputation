...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.n_nodes = args.features
        self.input_dim = args.features
        self.output_dim = args.features
        self.in_steps = args.seq_len
        self.out_steps = args.seq_len

        self.input_embedding_dim = args.ImputeFormer_d_input_embed
        self.learnable_embedding_dim = args.ImputeFormer_d_learnalbe_embed
        self.model_dim = self.input_embedding_dim + self.learnable_embedding_dim

        self.n_temporal_heads = args.ImputeFormer_n_temporal_heads
        self.num_layers = args.ImputeFormer_n_layers
        self.input_proj = nn.Linear(self.input_dim, self.input_embedding_dim)
        self.d_proj = args.ImputeFormer_d_proj
        self.d_ffn = args.ImputeFormer_d_ffn

        self.dropout = args.ImputeFormer_dropout
        self.ORT_weight = args.ImputeFormer_ORT_weight
        self.MIT_weight = args.ImputeFormer_MIT_weight
        ...
        self.saits_loss_func = SaitsLoss(self.ORT_weight, self.MIT_weight)

    def evaluate(self, batch:dict, training:bool=True) -> torch.Tensor:
        res = self.forward(batch, training)
        return res['loss']
    
    def impute(self, batch:dict, n_smaples:int=None) -> torch.Tensor:
